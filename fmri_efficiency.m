...
figure,sgtitle(sprintf('Fig 1.1 Fixed SOA = %d', SOA), 'FontSize', 14)
subplot(1,2,1)
...
figure,sgtitle(sprintf('Fig 1.2 Fixed SOA = %d', SOA), 'FontSize', 14)
subplot(1,2,1)
stairs(t,u,'LineWidth',2);
...
figure,sgtitle(sprintf('Fig 1.3 Stochastic, SOAmin = %d', SOAmin), 'FontSize', 14)
subplot(1,2,1)
stairs(t,u,'LineWidth',2);
set(gca,'YTick',[0 1],'XTick',xtick); xlabel('Time (s)'); title('Neural Activity')
...
figure, sgtitle(sprintf('Fig 1.4. Blocked, Event Model, Stim per Block = %d, SOAmin = %d', stim_per_block, SOAmin), 'FontSize', 14)
subplot(1,2,1)
stairs(t,u,'LineWidth',2);
set(gca,'YTick',[0 1],'XTick',xtick); xlabel('Time (s)'); title('Neural Activity')
...
figure, sgtitle(sprintf('Fig 1.5. Blocked, Epoch Model, Block Length = %d', block_length), 'FontSize', 14)
subplot(1,2,1)
...
figure, sgtitle(sprintf('Fig 1.6. Fourier Transform of Epoch Model, Block Length = %d', block_length), 'FontSize', 14)
subplot(1,3,1)
...
figure, sgtitle('Fig 1.7. Fourier Transform of Optimal Sinusoidal Design', 'FontSize', 14)
subplot(2,2,1)
plot(t,u,'LineWidth',2); 
...
figure, sgtitle(sprintf('Fig 1.8. Example of aliasing'), 'FontSize', 14), hold on
plot(t,u,'LineWidth',2); 
...
figure, sgtitle(sprintf('Fig 1.9. Signal and Noise Spectra'), 'FontSize', 14), hold on
...
figure, sgtitle(sprintf('Fig 1.10. Blocked, Event Model, Stim per Block = %d, SOAmin = %d', stim_per_block, SOAmin), 'FontSize', 14), hold on
subplot(2,3,1)
stairs(t,u,'LineWidth',2);
set(gca,'YTick',[0 1],'XTick',xtick); xlabel('Time (s)'); title('Neural Activity')
...
figure, sgtitle(sprintf('Fig 1.11. Randomised design (again), SOAmin = %d', SOAmin), 'FontSize', 14); hold on
subplot(2,3,1)
stairs(t,u,'LineWidth',2);
set(gca,'YTick',[0 1],'XTick',xtick); xlabel('Time (s)'); title('Neural Activity')
...
figure, sgtitle('Fig 1.13. FIR estimates for Ran vs Blk designs', 'FontSize', 14)
...
figure, sgtitle('Fig 1.14. Design for randomised A,B (plus DCT high-pass filter)', 'FontSize', 14)
...
figure, sgtitle('Fig 1.15. Efficiency for 2 contrasts on 2 randomised event-types', 'FontSize', 14)
p = plot(SOAmin,eff);
linestyles = {':';'-'};
linecolors = {'r';'r'};
...
figure, sgtitle('Fig 1.16. Adding efficiency for alternating design...', 'FontSize', 14)
p = plot(SOAmin,eff);
linestyles{end+1} = '-';
linecolors{end+1} = 'b';
[p(:).LineStyle] = linestyles{:};
...
figure, sgtitle('Fig 1.18. Adding null events in a randomised design', 'FontSize', 14)
p = plot(SOAmin,eff);
linestyles{3} = {':'};
...
X = X/max(X(:));
figure, sgtitle(sprintf('Fig 1.19. Correlation between regressors, R = %3.2f', corr(X(:,1),X(:,2))), 'FontSize', 14)
hold on
...
figure,sgtitle('Fig 1.20. Geometric perspective on orthogonalisation', 'FontSize', 14)
...
figure, sgtitle('Fig 1.21. Separating Stimuli (S) and Responses (R)', 'FontSize', 14)
...
figure, sgtitle('Fig 22. Separating state vs item effects', 'FontSize', 14)
...
figure, sgtitle('Fig 2.1. Example LSA design matrix', 'FontSize', 14)
...
figure, sgtitle('Fig 2.2. Efficiency and Bias for LSA versus LSU', 'FontSize', 14)
for s=1:num_std
    fprintf('Trial:Scan std ratio = %3.2f\n',trl_std(s)/scn_std);
...
figure, sgtitle('Fig 2.3. Efficiency and Bias for LSA versus L2-regularised LSA', 'FontSize', 14)
for s=1:num_std
    fprintf('Trial:Scan std ratio = %3.2f\n',trl_std(s)/scn_std);
    m = mean(squeeze(est_diff(:,1,s)));
    d =  std(squeeze(est_diff(:,1,s)));
...
% mean. The "hyper-parameter" lambda controls the amount of regularisation (and 
% can be optimised with cross-validation).
% 
...
figure, sgtitle('Fig 2.4. Trial-wise correlation for LSA versus L2-regularised LSA', 'FontSize', 14)
for s=1:num_std
    if trl_std(s) > 0
        fprintf('Trial:Scan std ratio = %3.2f\n',trl_std(s)/scn_std);
        ...
        p = [p; m log10(1+1/d)];
        subplot(1,num_std,s)
        b = bar(p,'FaceColor','flat'); b(1).CData = repmat([0.0660 0.7450 0.4430],2,1); % Change Cor to green
        legend({'Corr', 'Eff'},'Location','NorthEast')
...
figure, sgtitle('Fig 2.5. Example LSS design matrices', 'FontSize', 14)
trl_lab = {};
...
figure, sgtitle('Fig 2.6. Efficiency and Bias for LSA versus LSS', 'FontSize', 14)
for s=1:num_std
    fprintf('Trial:Scan std ratio = %3.2f\n',trl_std(s)/scn_std(s));
    m = mean(squeeze(est_diff(:,1,s)));
    d =  std(squeeze(est_diff(:,1,s)));
...
figure, sgtitle('Fig 2.7. Trial-wise correlation for LSA versus LSS', 'FontSize', 14)
for s=1:num_std
    fprintf('Trial:Scan std ratio = %3.2f\n',trl_std(s)/scn_std(s));
    m = mean(squeeze(est_corr(:,1,s)));
    d =  std(squeeze(est_corr(:,1,s)));
...
figure, sgtitle('Fig 2.8. Difference between LSA and LSS in temporal smoothness', 'FontSize', 14)
...
%% 
% However, there are situations where the smoothness constraint of LSS does 
% introduce bias (again, no free lunch!). This is when the signal is temporally 
% auto-correlated. For example, imagine we wanted to estimate the difference in 
% response between trials of two types that alternate. This is simulated with 
% the code below (which may take a while...), which also compares two SOAs (2s 
% and 16s) and the situation where there is no variability between trials of the 
% same type (to maximise differences between LSS and LSA).

scn_std = 1;  
trl_std = 0;   % No trial variability
SOAs = [2 16]; % Two SOAs though note that 32s case will take a while to run!
num_soa = length(SOAs);

mB = repmat([1 2], 1, num_trl/2)'; % Alternating sequence of Betas differing in amplitude by 1 unit
v1 = ones(size(mB));               % Treat all trials same (LSS-1)
v2 = mB;                           % Distinguish trials by two conditions (LSS-2)

est_diff = nan(num_exp, 2, num_soa);
for s=1:num_soa
    fprintf('Computing with SOA = %d...\n',SOAs(s));
    transient = ceil(32/SOAs(s));            % Must be less than half num_trl!
    sv2 = v2((transient+1):(end-transient)); % Subset of trials after excluding transients
    for e=1:num_exp
        X_LSA = gen_X(1:num_trl, SOAs(s), HRF, TR);
        X_LSA = [X_LSA ones(size(X_LSA,1),1)];
        pX_LSA = pinv(X_LSA);
        ...
        B_hat_LSA = pX_LSA * y;
        B_hat_LSA = B_hat_LSA((transient+1):(end-transient-1)); % remove transient and constant term       
        est_diff(e,1,s) = mean(B_hat_LSA(sv2==2)) - mean(B_hat_LSA(sv2==1)); 

        B_hat_LSS = fit_lss(X_LSA, v1, y, transient);  % LSS-1
        % B_hat_LSS = fit_lss(X_LSA, v2, y, transient); % LSS-2
        est_diff(e,2,s) = mean(B_hat_LSS(sv2==2)) - mean(B_hat_LSS(sv2==1)); 
    end
end

figure, sgtitle('Fig 2.9. LSS issues for difference between trials close in time', 'FontSize', 14)
rng(1)
for s=1:num_soa
    fprintf('SOA = %d\n',SOAs(s));
    m = mean(squeeze(est_diff(:,1,s)));
    d =  std(squeeze(est_diff(:,1,s)));
    fprintf('\t LSA:     Diff=%3.2f (Z=%3.2f), Efficiency=%3.2f\n', m, m/d, 1/d);
    p = [m log10(1+1/d)]; 

    m = mean(squeeze(est_diff(:,2,s)));
    d =  std(squeeze(est_diff(:,2,s)));
    fprintf('\t LSS:     Diff=%3.2f (Z=%3.2f), Efficiency=%3.2f\n', m, m/d, 1/d);
    p = [p; m log10(1+1/d)];
    subplot(1,num_soa,s)
    bar(p); legend({'Diff', 'Eff'},'Location','SouthEast')
    set(gca,'XTickLabel',{'LSA','LSS'}); ylim([-2 2])
    title(sprintf('[1 -1], SOA = %d',SOAs(s)))
end
%% 
% Fig 2.9 shows that, at short SOAs (left panel), LSS estimates show no difference 
% in the mean of the two trial-types (the blue bar is zero), even though the true 
% difference is 1, as LSA correctly estimates. Thus, if you were only interested 
% in this difference in means, then the LSS bias becomes a critical problem (and 
% its higher efficiency irrelevant). At the long SOA (right panel), both estimators 
% are equivalent (i.e. the smoothing of LSS does not matter for such slow variation). 
% Indeed, the differences between LSA and LSS (and LSU and LSA-reg) are greatest 
% when the SOA is short, i.e., when there is high correlation between the individual 
% regressors in LSA and LSS models; when the SOA is long (eg 20+ seconds), they 
% give near-equivalent results.
% 
% Note that in the code above, the LSS model teated every trial as from the 
% same condition (what <https://doi.org/10.1016/j.neuroimage.2015.11.009 [Abdulrahman 
% & Henson, 2016]> called "LSS-1"), through passing "fit_lss" the stimulus vector 
% v1. If instead you pass "fit_lss" the stimulus vector v2 instead (uncommenting 
% the relevant line above) and re-run, you will no longer see any bias in the 
% LSS estimate, which will approach the correct estimate of a difference of 1. 
% This is because trials of the two conditions will be modelled in separate regressors 
% (what <https://doi.org/10.1016/j.neuroimage.2015.11.009 [Abdulrahman & Henson, 
% 2016]> called "LSS-N", where N=2 here). Thus LSS-N is generally advisable when 
% one is interested in the difference between N conditions, but sometimes neglected 
% when people fit a single LSS model treating every trial the same (or treating 
% every trial as a separate condition, e.g. in "condition-rich" designs).
% 
...
figure,sgtitle('Fig 3.1 Mean estimated pattern across trials', 'FontSize', 14)
for c = 1:2
    B_con{c} = B_hat(v==c, :);
...
R = [[eye(2*num_trl) zeros(2*num_trl,1)]; [zeros(1,2*num_trl) 0]];    % regularise all Betas but not grand mean
pX_LSA_reg = inv(X_LSA'*X_LSA + lambda*R)*X_LSA';   % Ridge regression or L2-norm regularisation

num_cov = length(c_trl);
est_wbc = nan(num_exp, 3, num_cov);
for s=1:num_cov
    ...
    for e=1:num_exp
        ...
        B_hat_LSA_reg = B_hat_LSA_reg((transient+1):(end-transient-1),:); % remove transients and constant term       

        for c = 1:2; B_con{c} = B_hat_LSA_reg(sv==c, :); end % reassign trial number to condition
        est_wbc(e,3,s) = wbc(B_con);
    end
end

figure, sgtitle('Fig 3.2. Within-Between as function of scan/trial voxel covariance', 'FontSize', 14)
rng(1)
for s=1:num_cov
    ...
    m = mean(squeeze(est_wbc(:,3,s)));
    d =  std(squeeze(est_wbc(:,3,s)));
    fprintf('\t LSA-reg: W-B=%3.2f (Z=%3.2f)\n', m, m/d);
    p = [p; m];

    subplot(2,2,s)
    bar(p); %legend({'W-B'},'Location','NorthEast')
    set(gca,'XTickLabel',{'LSA','LSS','LSA-reg'}); ylim([0 2.1]); xlim([0.5 3.5])
...
% is 2 for LSA and LSS, as expected (since correlation within pattern A or B is 
% 1, and between A and B is -1, so their difference is 2). With LSA-reg, the W-B 
% is less because of shrinkage of the estimates. When the scan noise is independent 
% across voxels (top right), then LSS does better than LSA (and LSA-reg is intermediate). 
% On the other hand, when trial variability is independent across voxels but scan 
% noise is perfectly correlated across voxels (bottom left), then LSA does better 
% than LSS (and LSA-reg). The bottom right shows the case of independent variability 
% across voxels for both trial and scan variability, where LSS (and LSA-reg) happen 
% to do better, though which estimator does better in general will depend on the 
% ratio of the total variability (ie, of trl_std:scn_std, as demonstrated in the 
% Section 2.3 - a ratio that just happens to be 1 in the above example - as well 
% as depending on lambda in the case of LSA-reg).
%% 
...
f1 = figure; sgtitle('Fig 3.3. Spatial covariation of trial vs scan variability with LSA', 'FontSize', 14)
f2 = figure; sgtitle('Fig 3.4. Spatial covariation of trial vs scan variability with LSS', 'FontSize', 14)
sp1 = [1 2 5 6];
sp2 = [3 4 7 8];
rng(1)
for s=1:num_cov
...
figure, sgtitle('Fig 4. BIR as function of neural duration (under linear assumptions)', 'FontSize', 14)
hold on
